    ...
    sources: [
      {
        ...
        page: null,
        section: "Conclusion",
        quote: "Optimal hardness achieved at 900°C",
      },
    ],
    gaps: [
      {
    ...
    sources: [
      {
        ...
        page: null,
        section: "Results",
        quote: "Corrosion rate at 25°C: 0.012 mm/year",
      },
    ],
    gaps: [
      {
    ...
    sources: [
      {
        ...
        page: null,
        section: "Conclusion",
        quote: "40 g/t PAX is the optimal dosage",
      },
    ],
    gaps: [
      {
...
module.exports = {
  ...
  async register(ctx) {
    ctx.router.post("/query", async (req, res) => {
      ...
      if (MOCK_ONLY) {
        ctx.log(`Mock response for user: ${payload.username}, query: "${query.slice(0, 50)}"`);
        await new Promise((r) => setTimeout(r, 400 + Math.random() * 400));
        return res.json(getMockResponse(query));
      }

      ctx.log(`Proxying query to agent backend for user: ${payload.username}`);
